...
class HomePayActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        confirmButton.setOnClickListener {
            if (!rentCheckbox.isChecked && !waterCheckbox.isChecked && !lightCheckbox.isChecked && !wifiCheckbox.isChecked) {
                // None of the checkboxes are checked, show an alert
                showAlert("Please check at least one option.")
            } else {
                // At least one checkbox is checked, proceed with payment confirmation and navigation
